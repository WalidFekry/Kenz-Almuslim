package com.benkkstudio.bsjson.utils;
...
public class BSShared {
    ...
    private BSShared(Context context) {
        preferences = PreferenceManager.getDefaultSharedPreferences(context);

    }

    public static synchronized BSShared getSharedPref(@NonNull Context context) {

        Log.d(TAG, Thread.currentThread().getName());
        if (sSharedPref == null) {
            Log.d(TAG,Thread.currentThread().getName());

            synchronized (BSShared.class) {
                Log.d(TAG,Thread.currentThread().getName());
                if (sSharedPref == null) {
                    Log.d(TAG,Thread.currentThread().getName());
                    sSharedPref = new BSShared(context);
                }
            }
